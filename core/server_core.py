...
class ServerCore:
    ...
    def stop_block_building(self):
        print('__generate_block_with_tp のスレッドは停止しています')
    ...
    def join_network(self):
        if self.core_node_host != None:
            ...
            print('このサーバは ネットワークの始祖 として実行されています...')
    ...
    def send_req_full_chain_to_my_peer(self):
        print('send_req_full_chain_to_my_central が呼び出されました')
    ...
    def get_all_chains_for_resolve_conflict(self):
        print('get_all_chains_for_resolve_conflict が呼び出されました')
        new_message = self.cm.get_message_text(MSG_REQUEST_FULL_CHAIN)
    ...
    def _check_availability_of_transaction(self, transaction):
        ...
        for used_o in used_outputs:
            ...
            if bm_v_result:
                print('このトランザクションアウトプットは使用済です', used_o)
                return False
            if tp_v_result:
                print('このトランザクションアウトプットはプール済です', used_o)
                return False
            if bm_v_result2 is not True:
                print('不明なトランザクションアウトプット', used_o)
                return False
    ...
    def get_total_fee_on_block(self, block):
        ...
        """
        print('get_total_fee_on_block が呼び出されました')
    ...
    def __core_api(self, request, message):
        ...
        if request == 'send_message_to_all_peer':
            ...
            if check_result:
                print('メッセージを送信... ', target_host, target_port)
    ...
    def __handle_message(self, msg, is_core, peer=None):
        if peer != None:
            ...
            if msg[2] == MSG_NEW_TRANSACTION:
                ...
                print('新しいトランザクション を受け取りました', new_transaction)
                ...
                if new_transaction in current_transactions:
                    print('プール済のトランザクション: ', new_transaction)
                    return
                else:
                    if not is_sbc_t:
                        print('Zeney のトランザクションではありません: ', new_transaction)
                    else:
                ...
                if self.bm.is_valid_block(self.prev_block_hash, new_block):
                    ...
                    if block_check_result is not True:
                        print('前ブロックのハッシュは正しいですが,まだ承認できません')
