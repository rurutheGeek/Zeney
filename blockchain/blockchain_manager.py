...
class BlockchainManager:
    ...
    def is_valid_block(self, prev_block_hash, block, difficulty=3):
        ...
        if block["previous_block"] != prev_block_hash:
            ...
            digest = binascii.hexlify(
                self._get_double_sha256((message + nonce).encode("utf-8"))
            ).decode("ascii")
                ...
                if t["t_type"] == "basic" or t["t_type"] == "coinbase_transaction":
                    if t["inputs"] != []:
                        ...
                        for it in inputs_t:
                            ...
                            if (
                                it["transaction"]["outputs"][it["output_index"]]
                                == transaction_output
                            ):
                                print(
                                    "This TransactionOutput was already used",
                                    transaction_output,
                                )
                                return True
    ...
    def is_valid_output_in_my_chain(self, transaction_output):
        ...
        """
        print("is_valid_output_in_my_chain が呼び出されました")
        current_index = 1
    ...
    def get_hash(self, block):
        ...
        return binascii.hexlify(
            self._get_double_sha256((block_string).encode("utf-8"))
        ).decode("ascii")
