...
class Zeney_Gui(Frame):
    ...
    def initApp(self, my_port, c_host, c_port):
        ...
        print('テスト用に 30 Zeney を付与します... ')
        self.um.extract_utxos([CoinbaseTransaction(self.km.my_address()).to_dict()])
    ...
    def update_callback(self):
        print('update_callback が呼び出されました')
    ...
    def create_menu(self):
        ...
        self.menuBar.add_cascade(label='メニュー', menu=self.subMenu)
        self.subMenu.add_command(label='自分のアドレスを表示', command=self.show_my_address)
        self.subMenu.add_command(label='キーの読み込み', command=self.show_input_dialog_for_key_loading)
        self.subMenu.add_command(label='ブロックチェーンの更新', command=self.update_block_chain)
        self.subMenu.add_command(label='ブロックチェーンを表示', command=self.show_my_block_chain)
        self.subMenu.add_separator()
        self.subMenu.add_command(label='終了', command=self.quit)

        self.subMenu2 = Menu(self.menuBar, tearoff=0)
        self.menuBar.add_cascade(label='設定', menu=self.subMenu2)
        self.subMenu2.add_command(label='キーの更新', command=self.renew_my_keypairs)        


    
    def show_my_address(self):
        f = Tk()
        label = Label(f, text='自分のアドレス')
        label.pack()
    ...
    def show_input_dialog_for_key_loading(self):
        ...
        def load_my_keys():
            ...
            messagebox.showinfo('キーペアをロード','キーファイルを選択してください')
        ...
        f = Tk()
        label0 = Label(f, text='キーペアのパスフレーズを入力してください')
        frame1 = ttk.Frame(f)
        label1 = ttk.Label(frame1, text='パスフレーズ:')
    ...
    def renew_my_keypairs(self):
        ...
        f = Tk()
        f.title('新しいキーの生成')
        label0 = Label(f, text='新しいキーペアのパスフレーズを入力してください')
        frame1 = ttk.Frame(f)
        label1 = ttk.Label(frame1, text='パスフレーズ:')
    ...
    def show_my_block_chain(self):
        ...
        if mychain is not None:
            ...
            self.display_info('現在のブロックチェーン', mychain_str)
        else:
            self.display_info('注意', '現在のブロックチェーンは空です...')
    ...
    def setupGUI(self):
        ...
        self.parent.title('ウォレットアプリ')
        self.pack(fill=BOTH, expand=1)

        self.create_menu()

        lf = LabelFrame(self, text='残高')
        ...
        #受信者となる相手の公開鍵
        self.label = Label(lf2, text='送信先アドレス:')
        ...
        # 送信額
        self.label2 = Label(lf2, text='送信額:')
        ...
        # 手数料
        self.label3 = Label(lf2, text='手数料 (オプション) :')
    ...
    def sendCoins(self):
        ...
        if not sendAtp:
            messagebox.showwarning('注意', '送信額を入力してください')
            return
        elif len(recipientKey) <= 1:
            messagebox.showwarning('注意', '送信先アドレスを入力してください')
            return
        else:
            result = messagebox.askyesno('確認', '{} Zeneyを以下のアドレスに送信します :\n {}'.format(sendAtp, recipientKey))
...
def main(my_port, c_host, c_port):
    ...
    root = Tk()
    app = Zeney_Gui(root, my_port, c_host, c_port)
    root.mainloop()
